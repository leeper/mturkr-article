...
\pkg{MTurkR}'s \code{seconds()} function provides a convenient way of converting time measurements in days, hours, minutes, or seconds into a total number of seconds. With the HITType created, one can begin creating individual HITs associated with that HITType using \code{CreateHIT()}.
...
deleted from the system using \code{DisposeHIT()}. This is not a
